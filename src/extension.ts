  ...
  const disposables = [
    ...
    vscode.commands.registerCommand("verticals.registerKeybindings", () => {
      vscode.window.showInformationMessage(
        "Keyboard shortcuts registered for Instagram Reels"
      );
    }),
  ];
...
async function loginToInstagram(
  ...
  try {
    ...
    const authUrl = `${INSTAGRAM_AUTH_URL}?client_id=${INSTAGRAM_CLIENT_ID}&redirect_uri=${encodeURIComponent(
    ...
    if (!authCode || authCode.trim() === "") {
      vscode.window.showWarningMessage(
        "Authorization code is required to login."
      );
      return;
    }
 ...
 */
async function promptForCustomFeed(
  context: vscode.ExtensionContext
): Promise<void> {
 ...
 */
async function promptForProductivityTimer(
  context: vscode.ExtensionContext
): Promise<void> {
  // Get the current timer setting
  const currentTimer = context.globalState.get<number>(
    "instagramTimerMinutes",
    15
  );
  ...
  if (minutes) {
    ...
    vscode.window.showInformationMessage(
      `Productivity timer set to ${timerMinutes} minutes`
    );
...
class ReelsPanel {
  ...
  private loadSettings(): void {
    this.productivityTimerMinutes = this.context.globalState.get<number>(
      "instagramTimerMinutes",
      15
    );
    this.accessToken = this.context.globalState.get<string>(
    ...
    if (!this.hashtag) {
      this.hashtag =
        this.context.workspaceState.get<string>("instagramHashtag");
    }
  }
  ...
  public static createOrShow(
    ...
    ReelsPanel.currentPanel = new ReelsPanel(
      panel,
      extensionUri,
      context,
      hashtag
  ...
  private async handleMessage(message: any): Promise<void> {
    try {
      switch (message.command) {
        ...
        case "timerExpired":
          vscode.window.showWarningMessage(
            "Your reel watching time limit has been reached!"
          );
          this.dispose();
          break;
        default:
   ...
   */
  public setHashtag(hashtag: string): void {
    this.hashtag = hashtag;
   ...
   */
  public setProductivityTimer(minutes: number): void {
   ...
   */
  public navigateReels(direction: number): void {
  ...
  private async fetchReelsByHashtag(hashtag: string): Promise<InstagramReel[]> {
    ...
    const hashtagResponse = await axios.get<HashtagResponse>(
      `${INSTAGRAM_GRAPH_BASE_URL}/ig_hashtag_search`,
      {
        params: {
          access_token: this.accessToken,
          q: hashtag,
    ...
    const response = await axios.get<InstagramResponse>(
      `${INSTAGRAM_GRAPH_BASE_URL}/${hashtagId}/top_media`,
      {
        params: {
          access_token: this.accessToken,
          fields:
   ...
   */
  private startProductivityTimer(): void {
   ...
   */
  private restartProductivityTimer(): void {
    this.clearProductivityTimer();
    this.startProductivityTimer();
   ...
   */
  private clearProductivityTimer(): void {
    if (this.timerHandle) {
  ...
  private update(): void {
    if (this.activeReels.length > 0) {
      this.panel.webview.html = this.getReelHtmlContent(
        this.activeReels[this.currentReelIndex]
      );
    }
  }
   ...
   */
  private showNoReelsMessage(): void {
    this.panel.webview.html = `
            <!DOCTYPE html>
            <html lang="en">
            <head>
  ...
  private getReelHtmlContent(reel: InstagramReel): string {
    const mediaUrl =
      reel.media_url || (reel.thumbnail_url ? reel.thumbnail_url : "");
